  ...
  Code: async (client: Actions, channel: string, userstate: Userstate, context: any[]) => {
    ...
    if (context[0]) {
      if (context[0] === "send") {
        ...
        if (found[0]) {
          return sendMessage(client, channel, `@${userstate.username} message: ${found[0].message} status: ${found[0].status}`);
        } else return sendMessage(client, channel, `@${userstate.username} couldn't find a suggestion with that ID`);
      }
    } else {
      let query = await findQuery('SELECT COUNT(*) FROM suggestions WHERE uid=?', [userstate['user-id']]);
      let total = (query[0]) ? query[0]['COUNT(*)'] : 0;
      sendMessage(client, channel, `@${userstate.username} you have submitted ${total} suggestions. If you'd like to send feedback use: ${currentSettings.prefix}suggest (send/check) (message/id)`);
    }
  }
}
