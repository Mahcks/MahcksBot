...
const statsCommand: CommandInt = {
  ...
  Code: async (client: Actions, channel: string, userstate: Userstate, context: any[]) => {
    ...
    if (/(command|cmd)/gi.test(cmd)) {
      // Gets how many times a command has been used.
      const used = await findQuery('SELECT count FROM commands WHERE name=?', [context[1]]);
      sendMessage(channel, `@${user} that command has been used ${used[0].count} times.`);

    } else if (/(channel)/gi.test(cmd)) {
      const targetChannel = (context[1]) ? context[1].replace('@', '') : channel.substring(1);
      const total = await logQuery(`SELECT COUNT(*) FROM logs.${targetChannel.toLowerCase()};`, []);

      const dataSize = await logQuery(`SELECT table_name AS 'table_name', ROUND((data_length + index_length)) AS 'size_in_bytes' FROM information_schema.TABLES WHERE table_schema = 'logs' AND table_name = '${targetChannel}'`, [])
      const size = parseInt(dataSize[0]["size_in_bytes"]) as number;
