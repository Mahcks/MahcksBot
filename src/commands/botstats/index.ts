...
const botstatsCommand: CommandInt = {
  ...
  Code: async (client: Actions, channel: string, userstate: Userstate, context: any[]) => {
    ...
    sendMessage(channel, `@${target} total commands: ${humanizeNumber(parseInt(totalCmds[0]['count(*)']))}, chatters logged: ${humanizeNumber(parseInt(totalChatters[0]['count(*)']))} commits farmed: ${humanizeNumber(parseInt(commits.toString('utf-8').replace("\n", "")))}`);
  }
}
