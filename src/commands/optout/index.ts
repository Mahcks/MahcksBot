...
const optoutCommand: CommandInt = {
  ...
  Code: async (client: Actions, channel: string, userstate: Userstate, context: any[]) => {
    ...
    if (checkCommand[0]) {
      ...
      if (!canOptout) return sendMessage(client, false, channel, `@${user} can't optout of that command.`);
      ...
      if (isUser[0]) {
        ...
        if (optedOut.includes(cmdSearch.toLowerCase())) {
          sendMessage(client, false, channel, `@${user} already opted out of ${cmdSearch.toLowerCase()}. If you'd like to opt back in use ${currSettings.prefix}optin ${cmdSearch.toLowerCase()}`);
        } else {
          ...
          sendMessage(client, false, channel, `@${user} opted out of ${cmdSearch.toLowerCase()}`);
        }
      } else {
    ...
    } else return sendMessage(client, false, channel, `@${user} couldn't find the command "${cmdSearch}"`);
  }
}
